...
class DataExtractor(object):
    ...
    def ExtractMinAndMaxValuesOfInputsAndOutputs(self, annotationsHolder):
        ...
        self.minAverageDistance, self.maxAverageDistance = DataExtractor.FindMinAndMaxInAnnotations(annotationsHolder, 'average distance')
        self.minWidthTop, self.maxWidthTop               = DataExtractor.FindMinAndMaxInAnnotations(annotationsHolder, 'width top')
        self.minWidthBottom, self.maxWidthBottom         = DataExtractor.FindMinAndMaxInAnnotations(annotationsHolder, 'width bottom')
    ...
    def ExtractAnnotationInputsAndOutputs(self, annotationsHolder):
        ...
        for i in range(self.numberOfImages):
            ...
            # Normalize the data
            currentAverageDistanceNorm = (currentAverageDistance - self.minAverageDistance)/(self.maxAverageDistance - self.minAverageDistance)
            currentWidthTopNorm        = (currentWidthTop - self.minWidthTop)/(self.maxWidthTop - self.minWidthTop)
            currentWidthBottomNorm     = (currentWidthBottom - self.minWidthBottom)/(self.maxWidthBottom - self.minWidthBottom)
