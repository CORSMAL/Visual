...
for n in range(configHolder.config['epochs']):
    ...
    for i in range(dataExtractorTraining.numberOfBatches):
        ...
        summaryCurrentEpochTraining.AppendValueInSummary('predictions',
                                                         predictedValuesBatchDenorm.cpu().detach().numpy())
    ...
    with torch.no_grad():
        ...
        for i in range(dataExtractorValidation.numberOfBatches):
            ...
            # Append prediction in summary
            summaryCurrentEpochValidation.AppendValueInSummary('predictions',
                                                               predictedValuesBatchDenorm.cpu().detach().numpy())
